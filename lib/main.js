var rootnodeTorrentManager = {
                            ...
                            if(response.response.type !== 'application/octet-stream') {
                               worker.port.emit('queueResult', {hash: data.hash, success: false, message: 'Cannot get file from torcache.net'});
                                ...
                                _this.sendAPIRequest.call(_this, {
                                    ...
                                    error: function(response) {
                                        worker.port.emit('queueResult', {hash: data.hash, success: false, message: response.error, authenticated: response.authenticated});
                                    }
                                });
                            }
                        }
    ...
    sendAjaxRequest: function (options) {
        ...
        xhr.onload = function() {
            if (xhr.readyState == 4) {
                if(xhr.response) {
                    options.authenticated = !(xhr.status === 403 || xhr.status === 401);
                    xhr.response.statusCode = xhr.status;
                    options.success({response: xhr.response, authenticated: options.authenticated});
                } else {
    ...
    // this document is an XUL document
        ...
        menuInvites.setAttribute('label', 'List available invite codes');
