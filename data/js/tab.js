...
self.port.on('queuedTorrentsList', function(list) {
    ...
    for(var l = 0; l < list.length; l += 1) {
        ...
        rows += '<tr>';
        rows += '<td class="title"><a href="view.html?' + list[l].url_hash + '" class="download' + (list[l].bytes_done !== list[l].size_bytes ? ' disabled' : '') + '">' + list[l].name + '</a> <span style="display: block; font-size: 12px;">Expires: '+ list[l].url_expires_at + ' | <a href="https://torrentz.ch/' + list[l].hash + '" target="_blank">Hash: ' + list[l].hash + '</a></span></td>';
